...
if (args[3]=='all') { NSNP='all' } else { NSNP= as.numeric(args[3]) }
if (length(args)>3) {
...
#-----------------------------------------------      
    ...
    p<-                  ggplot(correl, aes(x=Iteration)) +
                            ...
                            scale_color_manual(name="", values=c("darkviolet","red","green","blue"))+
                            scale_y_continuous(
...
#----------------------------------------------- 
    ...
    solaniTest<-       data.table(ID=A$can, ebv_m=EBVcan[,ncol(EBVcan)])
